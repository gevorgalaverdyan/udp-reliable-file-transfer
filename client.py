...
def rcv_file(server_ip: str, port: int, filename: str, segment_size: int):
    ...
    log(f"Requesting '{filename}' from {server_ip}:{port}  "
        f"(segment_size={segment_size})")
    
    req_packet = pack_packet(conn_id, 0, MESSAGE_TYPES.REQUEST, False, filename.encode())
    ...
    if not packet:
        log("ERROR: received packet is NONE")
        return
    
    connection_id, sequence_number, message_type, is_final, payload = packet
    if connection_id != conn_id:
        log(f"ERROR: connection ids not equal received={connection_id} expecting={conn_id}")
        return
    ...
    if message_type==MESSAGE_TYPES.ERROR:
        ...
        log(f"RECEIVED: seq={sequence_number}, payload_size={len(payload)}, is_final={is_final}")
        return
    else: 
...
def main():
    ...
    parser.add_argument("--segment-size", type=int, help=f"Max UDP payload bytes per DATA packet", required=True)
